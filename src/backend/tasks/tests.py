...
class TasksTest(TestCase):
    async def test_get_tasks(self):
        ...
        response = await client.post(f"/{test_user.tg_id}/start-completion/{test_task.id}")
        ...
        with patch("utils.get_time", return_value=timezone.now() + datetime.timedelta(
                seconds=settings.TASK_COMPLETION_PENDING_TIME + 10)):
            response = await client.get(f"/{test_user.tg_id}")
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.json()[0]["status"], "ready", response.json()[0])
            response = await client.post(f"/{test_user.tg_id}/start-completion/{test_task.id}")
